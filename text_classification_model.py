...
Original file is located at
    https://colab.research.google.com/drive/1gE7a3xwH-x883RCtviq8ICuVFgRIffjE
"""
...
train_iter = iter(AG_NEWS(split='train')) #creates iterator, 
#splits input into different elems at each word 'train' doesn't include 'train'
...
tokenizer = get_tokenizer('basic_english') #Generate tokenizer function for a string sentence, choose lang, splits by space for eng
train_iter = AG_NEWS(split='train') 

def yield_tokens(data_iter): 
    for _, text in data_iter: #ignore the first elem of tuple
        yield tokenizer(text) #return token

vocab = build_vocab_from_iterator(yield_tokens(train_iter), specials=["<unk>"])
vocab.set_default_index(vocab["<unk>"]) #param: index – Value of default index. This index will be returned when OOV token is queried.

#Which accepts iterator that yield list or iterator of tokens. 
...
text_pipeline = lambda x: vocab(tokenizer(x)) #look at description below
label_pipeline = lambda x: int(x) - 1         #convert label to integer
